...
while true; do
    ...
    do
        ...
        GRIB_2="temp_${FILE_FHR}_apcp.grib2"
        JSON="data/gfs_f${FILE_FHR}.json"
        JSON_2="data/gfs_rain_f${FILE_FHR}.json"
        ...
        URL_2="https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?dir=%2Fgfs.20260522%2F06%2Fatmos&file=gfs.t${CYCLE}z.pgrb2.0p25.f${FILE_FHR}&var_APCP=on&dir=%2Fgfs.${DATE}%2F${CYCLE}%2Fatmos"
        curl -f -s "$URL" -o "$GRIB"
        curl -f -s "$URL_2" -o "$GRIB_2"

        if [ -s "$GRIB" ]; then
            echo "📦 Converting to JSON..."
            JAVA_OPTS="-Xmx4g" "$GRIB2JSON" -d -c -n -p 2 --fp 2 -o "$JSON" "$GRIB"
            JAVA_OPTS="-Xmx4g" "$GRIB2JSON" -d -c -n -p 2 --fp 2 -o "$JSON_2" "$GRIB_2"
            rm -f "$GRIB"
            rm -f "$GRIB_2"
        else
